...
public class SendMailTemplate {
	...
	public boolean sendTemplate(String address, String user){
		...
		try {
			...
			logger.error("发送模版邮件异常: 不支持的编码: "+e.getMessage());
			return false;
		}
		...
		try {
			...
			logger.error("发送模版邮件异常: 客户端协议异常: " + e.getMessage());
			return false;
		} catch (IOException e) {
			logger.error("发送模版邮件异常: 输入输出异常: " + e.getMessage());
			return false;
		}
