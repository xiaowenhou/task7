package com.putaoteng.task7.utils.other;
...
public class SendMailContentSMTP {
	...
	public static void send(final String apiUser, final String apiKey, String address) {
		...
		try {
			...
			logger.error("普通邮件发送异常(SMTP): 无效的编码格式: "+e.getMessage());
		} catch (AddressException e) {
			logger.error("普通邮件发送异常(SMTP): 收信人地址异常: "+e.getMessage());
		} catch (MessagingException e) {
			logger.error("普通邮件发送异常(SMTP): Message异常: "+e.getMessage());
		}
		...
		try {
			...
			logger.error("普通邮件发送异常(SMTP): Message异常(setContent()方法): "+e.getMessage());
		}
		...
		try {
			...
			logger.error("普通邮件发送异常(SMTP): 发送失败异常: "+e.getMessage());
		} catch (NoSuchProviderException e) {
			logger.error("普通邮件发送异常(SMTP): 无提供者异常: "+e.getMessage());
		} catch (MessagingException e) {
			logger.error("普通邮件发送异常(SMTP): Message异常(getRecipients()方法): "+e.getMessage());
		}
	}
}
